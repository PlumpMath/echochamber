...
(ns glint.views.common
  (:require [cemerick.austin.repls :refer [browser-connected-repl-js]])
  (:use [hiccup.core :refer :all]))

(def navbar
  [:nav.navbar.navbar-default.main-navbar.navbar-fixed-top {:role "navigation"}
   ;; Brand and toggle get grouped for better mobile display
   [:div.navbar-header
    [:a.navbar-brand {:href "#"} [:img {:src "/images/icon128.png"}] " Glint"]]
...
(defn layout [& content]
  [:html
   [:head
    [:title "Glint - Design in the browser"]
    ;;[:link {:rel "stylesheet", :href "/css/vendor/fa/font-awesome.min.css"}]
